...
class Transformer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, relative_pos_embed=None):
        for i in range(len(self.layers)//6):
            ...
            attn_x, attn_m = attn(x, attn_mask=attn_mask, relative_pos_embed=relative_pos_embed)
            x = dp1(ls1(attn_x)) + x
            x = dp2(ls2(ff(x))) + x
...
class Attention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, relative_pos_embed=None):
        ...
        output = torch.matmul(attn_d, v).transpose(1, 2).reshape(b, n, h*hd)  # (b, h, n, hd) -> (b, n, h*hd)

        return self.to_out(output), attn  # (b, n, d)
